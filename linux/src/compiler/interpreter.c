...
static int handle_function_call(const char* path, int* pc_ptr, int end_tok, int argc, const char** argv) {
    ...
    for(int i=0; i < g_token_count; i++){
        if(g_tokens[i].type == TOKEN_KEYWORD && (strcmp(g_tokens[i].value, "fn") == 0 || strcmp(g_tokens[i].value, "tar_fn") == 0)){
    ...
    if(fn_pc != -1){
        ...
        if(p < g_token_count){
            ...
            // Map parameters to arguments - handle both old (type name) and new (name: type) syntax
            int pp = fn_pc + 2; // Skip name and (
            int cur_arg = 0;
            while(pp < g_token_count && g_tokens[pp].type != TOKEN_RPAREN){
                if(g_tokens[pp].type == TOKEN_IDENTIFIER){
                    char param_name[128]; strcpy(param_name, g_tokens[pp].value);
                    int param_line = g_tokens[pp].line, param_col = g_tokens[pp].col;
                    pp++;
                    
                    int is_new_syntax = 0;
                    if(pp < g_token_count && g_tokens[pp].type == TOKEN_AMPERSAND){
                        while(pp < g_token_count && g_tokens[pp].type != TOKEN_COLON && g_tokens[pp].type != TOKEN_COMMA && g_tokens[pp].type != TOKEN_RPAREN){
                            pp++;
                        }
                    }
                    if(pp < g_token_count && g_tokens[pp].type == TOKEN_COLON){
                        is_new_syntax = 1;
                        pp++;
                        while(pp < g_token_count && (g_tokens[pp].type == TOKEN_KEYWORD || g_tokens[pp].type == TOKEN_IDENTIFIER)){
                            pp++;
                            if(pp < g_token_count && strcmp(g_tokens[pp].value, "&&") == 0){
                                pp++;
                            } else {
                                break;
                            }
                        }
                    } else if(is_type_keyword(param_name)){
                        is_new_syntax = 0;
                    }
                    
                    if(is_new_syntax || !is_type_keyword(param_name)){
                        if(cur_arg < arg_cnt){
                            set_variable(param_name, args[cur_arg], TYPE_UNDEFINED, path, param_line, param_col);
                        }
                        cur_arg++;
                    }
                }
                
                if(pp < g_token_count && g_tokens[pp].type == TOKEN_COMMA){
                    pp++;
                } else if(pp < g_token_count && g_tokens[pp].type == TOKEN_RPAREN){
                    break;
                } else {
                    pp++;
                }
...
static int find_main_tokens(int* body_start, int* body_end){
    for(int i = 0; i < g_token_count; i++){
        if(g_tokens[i].type == TOKEN_KEYWORD && (strcmp(g_tokens[i].value, "fn") == 0 || (strcmp(g_tokens[i].value, "main") == 0 && (i == 0 || strcmp(g_tokens[i-1].value, "fn") != 0)))){
            int p = i;
            if(strcmp(g_tokens[i].value, "fn") == 0) p++;
            if(p < g_token_count && strcmp(g_tokens[p].value, "main") == 0){
                p++;
                while(p < g_token_count && g_tokens[p].type != TOKEN_LPAREN) p++;
                if(p < g_token_count){
                    while(p < g_token_count && g_tokens[p].type != TOKEN_RPAREN) p++;
                    if(p < g_token_count){
                        p++;
                        // Skip optional -> type or ->* type && type
                        if(p < g_token_count && g_tokens[p].type == TOKEN_ARROW){
                            p++;
                            if(p < g_token_count && g_tokens[p].type == TOKEN_STAR) p++;
                            if(p < g_token_count){
                                g_current_expected_ret_type = parse_type(g_tokens[p].value);
                                p++;
                            }
                            while(p < g_token_count && strcmp(g_tokens[p].value, "&&") == 0){
                                p++;
                                if(p < g_token_count) p++;
                            }
                        }
    ...
    if (current_pc + 3 < end_tok && g_tokens[current_pc+1].type == TOKEN_LBRACKET && 
        ...
        if (var) {
            if (strcmp(g_tokens[current_pc+2].value, "addr") == 0) {
                ...
                *pc += 2;
                return addr_buf;
            } else if (strcmp(g_tokens[current_pc+2].value, "length") == 0 || strcmp(g_tokens[current_pc+2].value, "bytes") == 0) {
...
int execute_code_block(const char* path, int start_tok, int end_tok, int argc, const char** argv){
    ...
    while(pc < end_tok && !g_should_return){
        ...
        if(tok->type == TOKEN_KEYWORD && (strcmp(tok->value, "fn") == 0 || (strcmp(tok->value, "main") == 0 && (pc == 0 || strcmp(g_tokens[pc-1].value, "fn") != 0)))){
             pc++; // skip fn or main
             if(pc < end_tok && (g_tokens[pc].type == TOKEN_IDENTIFIER || g_tokens[pc].type == TOKEN_KEYWORD)) pc++; // skip name
             if(pc < end_tok && g_tokens[pc].type == TOKEN_LPAREN){
                 int depth = 1; pc++;
                 while(pc < end_tok && depth > 0){
                     if(g_tokens[pc].type == TOKEN_LPAREN) depth++;
                     else if(g_tokens[pc].type == TOKEN_RPAREN) depth--;
                     pc++;
                 }
             }
             if(pc < end_tok && g_tokens[pc].type == TOKEN_ARROW){
                 pc++;
                 if(pc < end_tok && g_tokens[pc].type == TOKEN_STAR) pc++;
                 while(pc < end_tok && (g_tokens[pc].type == TOKEN_KEYWORD || g_tokens[pc].type == TOKEN_IDENTIFIER)){
                     pc++;
                     if(pc < end_tok && strcmp(g_tokens[pc].value, "&&") == 0){
                        pc++;
                     } else {
                        break;
                     }
        ...
        if(tok->type == TOKEN_KEYWORD && strcmp(tok->value, "tar_fn") == 0){
             pc++; // skip tar_fn
             if(pc < end_tok && (g_tokens[pc].type == TOKEN_IDENTIFIER || g_tokens[pc].type == TOKEN_KEYWORD)) pc++; // skip name
             if(pc < end_tok && g_tokens[pc].type == TOKEN_LPAREN){
                 int depth = 1; pc++;
                ...
                if(pc < end_tok && g_tokens[pc].type == TOKEN_LBRACE){
                    ...
                    while(pc < end_tok && g_tokens[pc].type != TOKEN_RBRACE){
                        if(g_tokens[pc].type == TOKEN_STRING){
                            ...
                            char full_path[512]; strcpy(full_path, inc_file);
                            int found = (read_welt_file(full_path, &code, &sz) == 0);
                            if(!found){
                                char fsal_root[512]; get_fsal_dir(fsal_root, sizeof(fsal_root));
                                snprintf(full_path, sizeof(full_path), "%s\\.fsal\\%s\\run.wt", fsal_root, inc_file);
                                found = (read_welt_file(full_path, &code, &sz) == 0);
                            }
                            if(found && code){
                                int start_before = g_token_count;
                                if(tokenize_append(full_path, code) == 0){
                                    int end_after = g_token_count;
                                    execute_code_block(full_path, start_before, end_after, argc, argv);
                                }
                                free(code);
                            }
                            pc++;
        ...
        if(tok->type == TOKEN_NOT && pc + 1 < end_tok && (g_tokens[pc+1].type == TOKEN_IDENTIFIER || g_tokens[pc+1].type == TOKEN_KEYWORD) && strcmp(g_tokens[pc+1].value, "include") == 0){
             ...
             if(g_tokens[pc].type == TOKEN_STRING){
                 ...
                 if(read_welt_file(inc_file, &code, &sz) == 0 && code){
                     int start_before = g_token_count;
                     if(tokenize_append(inc_file, code) == 0){
                         int end_after = g_token_count;
                         execute_code_block(inc_file, start_before, end_after, argc, argv);
                     }
                     free(code);
                 }
             }
        ...
        if(is_type_keyword(tok->value)){
            ...
            if(pc < end_tok && g_tokens[pc].type == TOKEN_IDENTIFIER){
                ...
                if(pc < end_tok && g_tokens[pc].type == TOKEN_ASSIGN){
                    ...
                    } else if(pc < end_tok && g_tokens[pc].type == TOKEN_KEYWORD && strcmp(g_tokens[pc].value, "ss_input") == 0){
                        pc++;
                        if(pc < end_tok && g_tokens[pc].type == TOKEN_LPAREN){
                            pc++; char prompt[256] = {0};
                            if(pc < end_tok && g_tokens[pc].type == TOKEN_STRING){ strcpy(prompt, g_tokens[pc].value); pc++; }
        ...
        if(tok->type == TOKEN_IDENTIFIER && pc + 1 < end_tok && g_tokens[pc+1].type == TOKEN_DOT){
            ...
            if(var){
                pc += 2;
                if(pc < end_tok && g_tokens[pc].type == TOKEN_IDENTIFIER){
                    char method[64]; strcpy(method, g_tokens[pc].value);
                    int method_pc = pc;
                    pc++;
                    if(pc < end_tok && g_tokens[pc].type == TOKEN_LPAREN){
                        pc++; char args[4][256]; int arg_count = 0;
                        int args_start_pc = pc;
                        while(pc < end_tok && g_tokens[pc].type != TOKEN_RPAREN){
                            if(g_tokens[pc].type == TOKEN_COMMA) { pc++; continue; }
                        ...
                        if(var->type == TYPE_FILE){
                            ...
                            if(strcmp(method, "has") == 0 && arg_count >= 1){ is_builtin = 1; set_variable("__last_ret", strstr(var->value, args[0]) ? "true" : "false", TYPE_BOOL, path, tok->line, tok->col); }
                            else if(strcmp(method, "length_of_variable") == 0){ is_builtin = 1; char buf[32]; snprintf(buf, sizeof(buf), "%zu", strlen(var->value)); set_variable("__last_ret", buf, TYPE_INTEGER, path, tok->line, tok->col); }
                            else if(strcmp(method, "bytes") == 0){ is_builtin = 1; set_variable("__last_ret", var->value, TYPE_STRING, path, tok->line, tok->col); }
                            else if(strcmp(method, "addr") == 0){ is_builtin = 1; char buf[64]; snprintf(buf, sizeof(buf), "0x%p", (void*)var->value); set_variable("__last_ret", buf, TYPE_STRING, path, tok->line, tok->col); }
                            else if(strcmp(method, "addToTable") == 0 && arg_count >= 1){ is_builtin = 1; strcat(var->value, "|"); strcat(var->value, args[0]); }
                            else if(strcmp(method, "addToArray") == 0 && arg_count >= 1){ is_builtin = 1; strcat(var->value, ","); strcat(var->value, args[0]); if(arg_count >= 2){ strcat(var->value, ":"); strcat(var->value, args[1]); } }
                        }

                        if(!is_builtin){
                            // Try to find a tar_fn with this name
                            int tar_pc = -1;
                            for(int i=0; i < g_token_count; i++){
                                if(g_tokens[i].type == TOKEN_KEYWORD && strcmp(g_tokens[i].value, "tar_fn") == 0){
                                    if(i+1 < g_token_count && strcmp(g_tokens[i+1].value, method) == 0){
                                        tar_pc = i; break;
                                    }
                                }
                            }
...
static int find_all_functions(FuncRange* ranges, int* count){
    *count = 0;
    for(int i = 0; i < g_token_count && *count < 100; i++){
        if(g_tokens[i].type == TOKEN_KEYWORD && (strcmp(g_tokens[i].value, "fn") == 0 || strcmp(g_tokens[i].value, "tar_fn") == 0 || (strcmp(g_tokens[i].value, "main") == 0 && (i == 0 || strcmp(g_tokens[i-1].value, "fn") != 0)))){
            int p = i + 1;
            while(p < g_token_count && g_tokens[p].type != TOKEN_LBRACE) p++;
            if(p < g_token_count){
...
static int find_top_level_code(int* start, int* end, FuncRange* ranges, int range_count){
    *start = -1; *end = -1;
    for(int i = 0; i < g_token_count; i++){
        ...
        if(!is_in_function(i, ranges, range_count)){
            if(*start == -1) *start = i;
            *end = i;
        }
    }

...
int is_keyword(const char* word){
    const char* keywords[] = {
        ...
        "Extension", "include", "tar_fn",
        "integer", "float", "string", "bool", "ss", "class", "table", "array",
        "sys_ind", "bit", "imp", "lib", "File", "impdef",
        "enum", "fwww_at", "sa_free", "has", "addToTable", "addToArray",
        "def_fsal", "execute_C", "Throw", "control-prog", "retype"
    };
...
static int tokenize_internal(const char* path, const char* src, int append){
    if(!append) g_token_count = 0;
        ...
        if(*p == '('){ tok->type = TOKEN_LPAREN; strcpy(tok->value, "("); p++; col++; }
            ...
            while(*p && i < 510){
                if(*p == '\\' && *(p+1)){
                    ...
                    switch(*p){
                        case 'n': tok->value[i++] = '\n'; p++; col++; break;
                        case 't': tok->value[i++] = '\t'; p++; col++; break;
                        case 'r': tok->value[i++] = '\r'; p++; col++; break;
                        case '\\': tok->value[i++] = '\\'; p++; col++; break;
                        case '"': tok->value[i++] = '"'; p++; col++; break;
                        case '0': tok->value[i++] = '\0'; p++; col++; break;
                        default: tok->value[i++] = '\\'; tok->value[i++] = *p; p++; col++; break;
                    }
                } else if(*p == '"') break;
                else {
                    tok->value[i++] = *p; if(*p == '\n') line++; p++; col++;
...
int tokenize(const char* path, const char* src){
    return tokenize_internal(path, src, 0);
}

int tokenize_append(const char* path, const char* src){
    if(g_token_count > 0 && g_tokens[g_token_count-1].type == TOKEN_EOF) g_token_count--;
    return tokenize_internal(path, src, 1);
}
